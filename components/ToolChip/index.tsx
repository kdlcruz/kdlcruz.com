// import { chipStatus, Tech } from "@/data/types";
...
// export const ToolChip = ({ tech, disabled }: ToolChipProps) => {
//   const colorClasses = {
...
//       : colorClasses[chipStatus[tech.level]]
//   }`} />
...
const customTheme: CustomFlowbiteTheme["badge"] = {
  root: {
    ...
    color: {
      info: "bg-blue-500 text-white group-hover:bg-cyan-200 dark:bg-cyan-200 dark:text-cyan-800 dark:group-hover:bg-cyan-300",
      ...
      warning:
        "bg-gray-500 text-white group-hover:bg-yellow-200 dark:bg-yellow-200 dark:text-yellow-900 dark:group-hover:bg-yellow-300",
      indigo:
...
export const ToolChip = ({ tech }: ToolChipProps) => {
  return (
    <Badge theme={customTheme} color={chipStatus[tech.level]} size="sm">
      {tech.name}
    </Badge>
  );
};
